...
def has_solved_riddle(request: Request):
    if "riddle_solved" in request.session and request.session["riddle_solved"]:
        return True
    else:
        raise HTTPException(status_code=303, detail="Riddle not solved", headers={"Location": "/riddle"})
